...
**Claude Code plugins (`type: "claude-plugin"`, e.g. wicked-garden) are not installed by `install-claude` at all.** Copying one into `skills/` or wiring its hooks produces a copy Claude Code never loads; registering it needs Claude Code's own plugin CLI. So the script reports such a product as a manual step (`skipped: true`, "registered by the central installer, not copied by this script"), stages nothing, writes nothing and records no marker entry — and stays self-contained (§15). The **central picker** (§14) hands `install-claude.js` the product list *without* plugins, then registers each plugin through the shared mechanism in `src/claude-plugin.ts` — probe (`claude --version` + on-disk state), `claude plugin marketplace add <source>` only when absent, `claude plugin install|update <plugin>@<marketplace>`, every call with `CLAUDE_CONFIG_DIR=<target>`, then a from-disk verdict (marketplace entry + install record + payload at `plugins/cache/<marketplace>/<plugin>/<version>/` with a matching manifest version) — and only after that succeeds removes a legacy skills/hooks copy an earlier script install recorded, by invoking the script's own `status` (read-only) and `uninstall` verbs (§12.3, containment-checked). The script's `uninstall` of a plugin product removes only such a legacy copy and names `claude plugin uninstall <plugin>@<marketplace>` for the registration itself. `claude plugin … list` is never used to read state (it writes `<configDir>/.claude.json`); a dry run spawns only the `claude --version` probe.
...
- `dir`/`file`: exists ⇒ ownership/signature check (§8.5) ⇒ remove recursively. Never remove shared discovery dirs (`skills/`, `agents/`, `commands/`) themselves; prune `wicked-installer/products/<id>` and backups per retention. **The marker is data on disk, not an instruction**: a recorded path is removable only when it is relative, has no `..` segment, sits under `skills/`, `agents/`, `commands/` or `wicked-installer/products/<id>/`, and — when it exists — realpath-resolves inside the config dir; a `json-key`/`hooks-entry` file must be the target's MCP state file or `settings.json` **and really be that file** — absent or a regular file, never a symlink (which would redirect the read, backup and replace), with `settings.json` realpath-contained in the config dir (the MCP file's default `~/.claude.json` is the documented exception). A `hooks-entry` is honoured only when its `ownerMatch.commandContains` is exactly this product's owner key (`wicked-installer/products/<id>`, what install wrote) and its `event` is a well-formed name; a `json-key` only when its pointer names `/mcpServers/<name>` — otherwise an empty or foreign selector would match, and delete, a user's own hooks or keys. Anything else is refused with a `skipped` action and a `refused: …` diagnostic, never removed. The same file check gates the install-side `mcp`/hooks wiring (§8.3/§8.4): a symlinked target yields a `failed` action and a note, and nothing is written.
