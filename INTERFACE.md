...
- `dir`/`file`: exists ⇒ ownership/signature check (§8.5) ⇒ remove recursively. Never remove shared discovery dirs (`skills/`, `agents/`, `commands/`) themselves; prune `wicked-installer/products/<id>` and backups per retention. **The marker is data on disk, not an instruction**: a recorded path is removable only when it is relative, has no `..` segment, sits under `skills/`, `agents/`, `commands/` or `wicked-installer/products/<id>/`, and — when it exists — realpath-resolves inside the config dir; a `json-key`/`hooks-entry` file must be the target's MCP state file or `settings.json` **and really be that file** — absent or a regular file, never a symlink (which would redirect the read, backup and replace), with `settings.json` realpath-contained in the config dir (the MCP file's default `~/.claude.json` is the documented exception). Anything else is refused with a `skipped` action and a `refused: …` diagnostic, never removed. The same check gates the install-side `mcp`/hooks wiring (§8.3/§8.4): a symlinked target yields a `failed` action and a note, and nothing is written.
