...
| `cargo` | `cargo install <install.crates[] ?? install.crate ?? install.package>` — `install.crates` (array) wins when present and installs every listed crate in ONE invocation (a multi-crate product like wicked-estate + wicked-estate-mcp is broken if only one lands); `install.crate` still names the primary/MCP binary for registration, detection, and uninstall. Pinning: `--version <install.version>` for a single crate; `name@version` per crate when several. `cargo` not on PATH → error `cargo not found; install Rust from https://rustup.rs and retry`. |
