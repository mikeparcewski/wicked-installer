...
| `cargo` | `cargo install <install.crates[] ?? install.crate ?? install.package>` — `install.crates` (array) wins when present and installs every listed crate in ONE invocation (a multi-crate product like wicked-estate + wicked-estate-mcp is broken if only one lands); `install.crate` still names the primary/MCP binary for registration. Detection and uninstall follow the same precedence: with `install.crates`, EVERY crate's own binary must resolve (each wicked-* crate's `[[bin]]` is named exactly like the crate — no `-mcp`-strip fallback, so a partial install reads not-installed and a re-run repairs it) and uninstall removes every listed crate; single-crate products keep keying on `install.crate` (with the historical differently-named-binary fallback in detection). Pinning: `--version <install.version>` for a single crate; `name@version` per crate when several. `cargo` not on PATH → error `cargo not found; install Rust from https://rustup.rs and retry`. |
...
The v1 flow (§5–7), plus: marker v2 bookkeeping (§10.2), MCP/hooks wiring where supported (§8.3/§8.4), stale-sweep on upgrade (§8.1), and the **REQUIRED stale-artifact purge (§12.4)**. An existing marker that is **unusable** — does not parse, or sits behind a symlink anywhere in `wicked-installer/claude-install.json` (parents included), or is not a regular file — **fails the install closed**: exit 1 before any staging, copying, deletion or marker write (also under `--dry-run`), with a diagnostic naming the file and the cause; nothing at that path is ever followed, created, renamed or rewritten (`status` reports it on stderr and exits 1 whatever the product selection — also with no ids or `--all`, and even when the marker link is dangling — and `uninstall` skips that target). **Every path below the config dir this script touches is walked no-follow** (`lstatChainNoFollow`): marker discovery/read/write, `skills/`, `agents/`, `commands/`, the hooks payload under `wicked-installer/products/`, `settings.json`, the MCP state file and the backups dir; a symlink in any component is refused with a typed error, and only a genuinely absent component reads as "absent". The product's own source manifests (`hooks/hooks.json`, `skills/*/SKILL.md`, the `platform/claude` override, `package.json`) are walked no-follow from the staged source root as well — one behind a symlink is never read, and that skill / hook set is skipped with a named action. Other paths outside the config dir (the rest of the staging/temp trees, the registry, `~/.local/bin`) are not subject to this walk.

**Claude Code plugins (`type: "claude-plugin"`, e.g. wicked-garden) are not installed by `install-claude` at all.** Copying one into `skills/` or wiring its hooks produces a copy Claude Code never loads; registering it needs Claude Code's own plugin CLI. So the script reports such a product as a manual step (`skipped: true`, "registered by the central installer, not copied by this script"), stages nothing, writes nothing and records no marker entry; its `status` does not report plugins (a marker entry for one can only be a legacy copy), and its `uninstall` of one **removes nothing** — the notice names `claude plugin uninstall <plugin>@<marketplace>` for the registration itself and leaves any legacy copy (and its marker record) in place, byte-identical (issue #20) — the script stays self-contained (§15). A v1 marker is upgraded to v2 with every legacy product entry carried forward (no file manifest, noted as such), never dropped. The **central picker** (§14) hands `install-claude.js` the product list *without* plugins, then registers each plugin through the shared mechanism in `src/claude-plugin.ts` — probe (`claude --version` + on-disk state, every path component lstat'd), `claude plugin marketplace add <source>` only when absent, `claude plugin install|update <plugin>@<marketplace>`, every call with `CLAUDE_CONFIG_DIR=<target>`, then a from-disk verdict (marketplace entry + install record with a real version + payload at `plugins/cache/<marketplace>/<plugin>/<version>/` with a matching manifest version). If the run added the marketplace and the install then fails, the add is rolled back with `claude plugin marketplace remove`. Legacy copies an earlier installer left behind — the bare `plugins/<plugin>/` copy, or a skills/hooks copy recorded in this script's marker — are **detected and reported, never removed** ("legacy … copy detected at … — left in place; removal will ship separately", tracked in issue #20). `claude plugin … list` is never used to read state (it writes `<configDir>/.claude.json`); a dry run spawns only the `claude --version` probe.
